...
def predict_one_complex(affinity_pred, df, orig_complex_graph, model, tr_schedule, rot_schedule, tor_schedule, res_tr_schedule, res_rot_schedule, res_chi_schedule,
    ...
    for rank, order in enumerate(re_order):
        if rank == 0:
            best_order = order
        # 重命名文件
        new_name = ligandFiles[order].replace(f'step1_rank{order+1}',f'rank{rank+1}')
        os.rename(ligandFiles[order], new_name)

        # 更新 ligandFiles 数组
        ligandFiles[order] = new_name
    ...
    if os.path.exists('eval_metrics.csv'):
        ...
        # 创建一个新的 DataFrame
        df = pd.DataFrame(columns=['referligand'])

    # 添加新的行
    ori_lig = ligand_path
    infer_rk1 = ligandFiles[best_order]
    ...
    new_row = {'referligand': ori_lig, 'inferligand': ligandFiles[best_order], 'clash': clash_scores[best_order], 'rmsd': rmsd, 'rmsd_sym': rmsd_sym, 'lddt': all_lddt_pred[best_order]}
